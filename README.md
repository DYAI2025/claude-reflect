...
Claude reasons: *"These 4 requests have the same intent - reviewing productivity data. The workflow is: fetch ActivityWatch → categorize time → calculate focus score. This is a strong candidate for /daily-review."*
...
1. /daily-review (High) — from bayram-os
   ...
   Corrections learned: "use Pacific timezone", "Telegram can be work"

2. /campaign-analytics (High) — from edu-website
   → Analyze marketing campaigns via PostHog
   Evidence: 10 similar requests
   Corrections learned: "use properties.utm_campaign"

════════════════════════════════════════════════════════════

Which skills should I generate?
> [1] /daily-review, [2] /campaign-analytics

Where should each skill be created?
┌──────────────────────┬─────────────────────┐
│ /daily-review        │ bayram-os           │
│ /campaign-analytics  │ edu-website         │
└──────────────────────┴─────────────────────┘

Skills created:
  ~/GH/bayram-os/.claude/commands/daily-review.md
  ~/GH/edu-website/.claude/commands/campaign-analytics.md
```
...
```markdown
---
description: Review daily productivity and focus using ActivityWatch
allowed-tools: Bash, Read, WebFetch
---

## Context
ActivityWatch analyzer at ~/GH/activitywatch-analysis-skill/
User timezone: Pacific

## Your Task
Analyze productivity for the specified day.

### Steps
1. Fetch ActivityWatch data for the day
2. Categorize by work type (deep work, meetings, admin)
3. Calculate focus score
4. Compare to previous days

### Guardrails
- Use Pacific timezone for date calculations
- Telegram can be productive (content creation) - ask if unclear
- Check AW Chrome plugin for browser details
